...
export const getDescription = (element: cheerio.Cheerio, item: string[], $: cheerio.Root) => {
...
export const validateElement = (element: string, getCountryText: string): string => {
...
export const duplicateResultsChecker = (results: ThingsToDo[], location: string, item: string[]) => {
...
export function getCountryText(element: cheerio.Cheerio, $: cheerio.Root): string | null {
    ...
    return /^\(\d+\)$/.test(nextText) ? `${countryText} ${nextText}`.trim() : countryText || null;
}